...
from svd_util.ext.byteplay import Code, opmap
...
def get_attr_usage( func, attrpath ):
    ...
    if not isinstance( func, Code):
        code = Code.from_code( getattr( func, 'im_func', func).__code__)
    else: code = func
    #print code.code
    root_var = attrpath[0]
    subattrs = attrpath[1:]
...
def execode( func, noargs =True, get_all_data =False, novarargs =True):
    ...
    fc = getattr( func, 'im_func', func).__code__
    ...
    #print code.__dict__
    if novarargs:
        code.varargs = code.varkwargs = False
    else:
...
'''
    ...
    @property
    def boundto( me):
        if me.ismethod: return me.orgfunc.__self__
        return None
    ...
    def call( me):
        ...
        me.locals.update( (k,v) for k,v in me.arg_defaults.items() if k not in me.locals)
        return eval( me.code, me.locals)
...
if __name__ == '__main__':
    ...
    cf = execode( f1)
    exec(cf, dlocalz( x=7, y=6, max= mymax ))
    #no return value!
    ...
    if 0:
        ...
        f2.__code__ = execode( f1, )#noargs=False)
        x=5;y=2; max=mymax
        print(f2())# x=8, y=1)   #

        print(eval( f2.__code__, dlocalz( x=9, y=0, max= mymax ) ))# x=8, y=1)
