#sdobrev 2010
'fixed variant of py3:html.parser: m = attrfind_tolerant.search(rawdata[:endpos], k)'
    ...
    def parse_starttag(self, i):
        ...
        while k < endpos:
            if self.strict:
                ...
                m = attrfind_tolerant.search(rawdata[:endpos], k)
            #print ( 3333333, endpos, m and m.end() )
            if not m:
                break
        ...
        if end not in (">", "/>"):
            ...
            if self.strict:
                self.error("junk characters in start tag: %r"
                           % (rawdata[k:endpos][:20],))
            #print( 222222222, tag, attrs, end, k,endpos)
